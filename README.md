...
- **The dark side**: every agent has an alignment (☀ +100 … −100 ☠, starts +50). Merits pull toward the light; failed tool calls, shipping edits without tests, flailing through 15+ tool calls, `--no-verify`, force-pushes, `rm -rf`, `reset --hard`, `sudo`, `|| true` pull toward the dark. Below −20 you are *tempted* (saber tints orange-red); below −50 you fall — *Sith Apprentice*, red saber, and your name becomes **Darth …**; below −80, *Sith Lord*. Sith quotes replace the holocron. The Council may `jedi redeem <agent>` (+40) — otherwise only clean, verified work brings you back.
